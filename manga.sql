...
--
-- Hôte : localhost:3306
-- Généré le : jeu. 27 mars 2025 à 13:31
...
--
-- Structure de la table `manga_info`
--
...
--
-- Déchargement des données de la table `manga_info`
--

INSERT INTO `manga_info` (`Titre`, `Date_Publication`, `Id`, `Note`, `Description`, `Personnage_Id`, `Auteur_Id`) VALUES
('le test', '1979-12-05', 1, 2, 'test du test', 1, 1),
('Berserk', '1989-08-26', 2, 9.47, 'Guts, a former mercenary now known as the Black Swordsman, is out for revenge. After a tumultuous childhood, he finally finds someone he respects and believes he can trust, only to have everything fall apart when this person takes away everything important to Guts for the purpose of fulfilling his own desires. Now marked for death, Guts becomes condemned to a fate in which he is relentlessly pursued by demonic beings.\r\n\r\nSetting out on a dreadful quest riddled with misfortune, Guts, armed with a massive sword and monstrous strength, will let nothing stop him, not even death itself, until he is finally able to take the head of the one who stripped him—and his loved one—of their humanity.', 2, 2),
...
--
-- Index pour la table `manga_info`
--
...
--
-- AUTO_INCREMENT pour la table `manga_info`
--
ALTER TABLE `manga_info`
...
--
-- Contraintes pour la table `manga_info`
--
ALTER TABLE `manga_info`
  ADD CONSTRAINT `manga_info_ibfk_1` FOREIGN KEY (`Auteur_Id`) REFERENCES `auteur` (`Id`),
  ADD CONSTRAINT `manga_info_ibfk_2` FOREIGN KEY (`Personnage_Id`) REFERENCES `personnage` (`Id`);
COMMIT;
